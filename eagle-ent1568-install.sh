...
sudo git clone --depth 1 --branch $OE_VERSION https://github.com/ShaheenHossain/jason_alpha_15ent $OE_HOME_EXT/
...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    sudo su root -c "printf 'addons_path=${OE_HOME_EXT}/alpha/addons,${OE_HOME}/custom/addons\n' >> /etc/${OE_CONFIG}.conf"
fi
...
sudo su root -c "echo 'sudo -u $OE_USER $OE_HOME_EXT/alpha-bin --config=/etc/${OE_CONFIG}.conf' >> $OE_HOME_EXT/start.sh"
...
echo "Addons folder: $OE_USER/$OE_CONFIG/alpha/addons/"
