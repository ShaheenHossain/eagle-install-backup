...
sudo -H pip3 install -r https://github.com/ShaheenHossain/eagle13.02/raw/${OE_VERSION}/requirements.txt
...
EOF
  ...
  sudo mv ~/eagle1348 /etc/nginx/sites-available/
  sudo ln -s /etc/nginx/sites-available/eagle1348 /etc/nginx/sites-enabled/eagle1348
  sudo rm /etc/nginx/sites-enabled/default
  sudo service nginx reload
  sudo su root -c "printf 'proxy_mode = True\n' >> /etc/${OE_CONFIG}.conf"
  echo "Done! The Nginx server is up and running. Configuration can be found at /etc/nginx/sites-available/eagle1348"
else
...
if [ $INSTALL_NGINX = "True" ]; then
  echo "Nginx configuration file: /etc/nginx/sites-available/eagle1348"
fi
echo "-----------------------------------------------------------"
