...
sudo adduser --system --quiet --shell=/bin/bash --home=$OE_HOME --gecos 'EAGLE1564' --group $OE_USER
...
sudo git clone --depth 1 --branch $OE_VERSION https://github.com/ShaheenHossain/slmnbnz_15ent $OE_HOME_EXT/
...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    GITHUB_RESPONSE=$(sudo git clone --depth 1 --branch $OE_VERSION https://github.com/ShaheenHossain/baramej_15ent "$OE_HOME/enterprise/addons" 2>&1)
...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    sudo su root -c "printf 'addons_path=${OE_HOME_EXT}/ubunzu/addons,${OE_HOME}/custom/addons\n' >> /etc/${OE_CONFIG}.conf"
fi
...
sudo su root -c "echo 'sudo -u $OE_USER $OE_HOME_EXT/ubunzu-bin --config=/etc/${OE_CONFIG}.conf' >> $OE_HOME_EXT/start.sh"
...
DAEMON=$OE_HOME_EXT/ubunzu-bin
NAME=$OE_CONFIG
DESC=$OE_CONFIG
# Specify the user name (Default: ubunzu).
USER=$OE_USER
...
if [ $INSTALL_NGINX = "True" ]; then
  ...
  # Add Headers for ubunzu proxy mode
...
EOF
  ...
  echo "Done! The Nginx server is up and running. Configuration can be found at /etc/nginx/sites-available/eagleent1564"
else
...
echo "Addons folder: $OE_USER/$OE_CONFIG/ubunzu/addons/"
...
if [ $INSTALL_NGINX = "True" ]; then
  echo "Nginx configuration file: /etc/nginx/sites-available/eagleent1564"
fi
echo "-----------------------------------------------------------"
